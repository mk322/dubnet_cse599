...
tensor delta_mean2d(tensor dy, tensor v)
{
    ...
    for(b = 0; b < n; ++b){
        for(k = 0; k < c; ++k){
            for(j = 0; j < h; ++j){
                for(i = 0; i < w; ++i){
                    dm.data[k] += dy.data[i + w*(j + h*(k + b*c))] * (-1/sqrt(v.data[k]+eps));
                }
            }
        }
    }
...
tensor delta_variance2d(tensor dy, tensor x, tensor m, tensor v)
{
    ...
    for(b = 0; b < n; ++b){
        for(k = 0; k < c; ++k){
            for(j = 0; j < h; ++j){
                for(i = 0; i < w; ++i){
                    dv.data[k] += -0.5 * dy.data[i + w*(j + h*(k + b*c))] * \
                                (x.data[i + w*(j + h*(k + b*c))] - m.data[k]) * \
                                pow(v.data[k]+eps, -1.5);
                }
            }
        }
    }
...
tensor delta_batchnorm2d(tensor dy, tensor dm, tensor dv, tensor m, tensor v, tensor x)
{
    ...
    for(b = 0; b < n; ++b){
        for(k = 0; k < c; ++k){
            for(j = 0; j < h; ++j){
                for(i = 0; i < w; ++i){
                    dx.data[i + w*(j + h*(k + b*c))] += dy.data[i + w*(j + h*(k + b*c))] * (1/sqrt(v.data[k]+eps)) + \
                                                        dv.data[k] * ((2*(x.data[i + w*(j + h*(k + b*c))] - m.data[k])) / num) + \
                                                        dm.data[k] / num;
                }
            }
        }
